...
public class Command implements Serializable {
    ...
    public static Command changeNameErrorCommand(String changeNameErrorMessage) {
        Command command = new Command();
        command.type = CommandType.CHANGENAME_ERROR;
        command.data = new ChangeNameErrorCommandData(changeNameErrorMessage);
        return command;
    }
    ...
    public static Command PasswordCommand(String oldPassword, String password) {
        Command command = new Command();
        command.type = CommandType.UPDATE_PASSWORD;
        command.data = new PasswordCommandData (oldPassword, password);
        return command;
    }
    public static Command PasswordErrorCommand(String PasswordError) {
        Command command = new Command();
        command.type = CommandType.UPDATE_PASSWORD_ERROR;
        command.data = new PasswordErrorCommandData(PasswordError);
        return command;
    }

    public static Command PasswordOkCommand(String password) {
        Command command = new Command();
        command.type = CommandType.UPDATE_PASSWORD_OK;
        command.data = new PasswordOkCommandData(password);
        return command;
    }
